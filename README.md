...
- **자동 갱신(선택)**: 선수기록 탭의 **득점 순위·도움 순위** — 아래 API-Football 키를 등록하면 자동 갱신됩니다.
- **자동 갱신 안 됨**: 경기별 선발 라인업·교체·카드 등 상세는 무료 API 범위 밖이라 `data.json` 시드값을 보존합니다(자동 갱신 시 덮어쓰지 않음). 직접 보강하려면 해당 경기의 `lineups`/`cards` 를 편집.
