...
# 일정: seasonId 엔드포인트로 시즌 전체 경기를 1회 호출(확인됨, [200]).
NAVER_SCHED = "https://api-gw.sports.naver.com/schedule/games?categoryId=worldcup&seasonId=" + NAVER_SEASON
...
def naver_schedule_index(dates=None):
    """월드컵 전체 경기목록 → {frozenset(영문팀쌍): game} 인덱스 (seasonId 1회 호출)."""
    idx = {}
    try:
        data = http_get(NAVER_SCHED)
    except Exception as e:
        print(f"  [naver] 경기목록 실패: {e}"); return idx
    ...
    print(f"  [naver] 경기목록 {len(games)}경기 로드")
    return idx
...
def main():
    ...
    # ── 경기결과 카드의 득점/도움/카드 — 네이버 경기 이벤트로 채움 ──
    # (선수순위와 별개 데이터. 완료된 조별 경기마다 득점자·도움·시간·카드 자동 채움)
    print("[+] 경기별 득점/도움 — 네이버 경기 이벤트 채우는 중…")
    try:
        fill_match_events_naver(out["matches"])
    except Exception as e:
        print(f"    → 네이버 경기 이벤트 오류(건너뜀, 기존값 유지): {e}")
