...
# ---- 경기별 득점/도움/카드: 네이버 경기 일정 + 이벤트 API ----  [경기결과 카드용]
# 일정: /schedule/games?categoryId=worldcup&date=YYYY-MM-DD  → result.games[] (gameId, 팀코드/명, 스코어, statusCode)
# 이벤트: /statistics/categories/worldcup/games/{gameId}/event?includeExtraInfo=true → result.events[]
#         eventType: Goal(득점, playerName=득점자 relatedPlayerName=도움), Booking(카드), Substitution(교체)
# 일정: fromDate/toDate(YYYY-MM-DD)로 날짜별 조회(확인됨). date=/seasonId=는 무시되고 '오늘'만 반환 → 사용 불가.
NAVER_SCHED_FMT = "https://api-gw.sports.naver.com/schedule/games?categoryId=worldcup&fromDate={d}&toDate={d}"
NAVER_EVENT = "https://api-gw.sports.naver.com/statistics/categories/worldcup/games/{gid}/event?includeExtraInfo=true"
NAVER_FINISHED = {"RESULT", "FINAL", "END", "ENDGAME"}   # 경기 종료 statusCode

def _nv_ko_en(name):
    """네이버 한글 팀명 → 대시보드 영문 팀명. 미매핑이면 한글명 그대로(국기만 미표시)."""
    n = (name or "").strip()
    return NV_KO2EN.get(n, n)

def naver_schedule_index(dates):
    """주어진 날짜들(YYYY-MM-DD)의 월드컵 경기목록 → {frozenset(영문팀쌍): game} 인덱스.
       fromDate/toDate 로 날짜별 조회(이 API는 날짜 범위만 인식)."""
    idx = {}; total = 0
    for d in sorted({x for x in (dates or []) if x}):
        try:
            data = http_get(NAVER_SCHED_FMT.format(d=d))
        except Exception as e:
            print(f"  [naver] 일정 {d} 실패: {e}"); continue
        games = ((data.get("result") or {}).get("games")) or []
        total += len(games)
        for g in games:
            he = _nv_ko_en(g.get("homeTeamName")); ae = _nv_ko_en(g.get("awayTeamName"))
            if not he or not ae:
                continue
            idx[frozenset([he, ae])] = {
                "gameId": g.get("gameId"), "status": g.get("statusCode"),
                "home_en": he, "away_en": ae,
                "hcode": g.get("homeTeamCode"), "acode": g.get("awayTeamCode"),
            }
    print(f"  [naver] 일정 {len(idx)}경기 매칭 (요청 {len(set(dates or []))}일, 응답 총 {total}경기)")
    return idx

def naver_game_events(game):
    """한 경기 event API → (goals, assists, cards) 대시보드 포맷. 실패 시 None."""
    gid = game["gameId"]
    try:
        data = http_get(NAVER_EVENT.format(gid=gid))
    except Exception as e:
        print(f"  [naver] 이벤트 {gid} 실패: {e}"); return None
    evs = ((data.get("result") or {}).get("events")) or []
    def team_en(code):
        if code and code == game.get("hcode"): return game["home_en"]
        if code and code == game.get("acode"): return game["away_en"]
        return None
    goals, assists, cards = [], [], []
    for e in evs:
        et = e.get("eventType"); tm = team_en(e.get("teamId"))
        if et not in ("Goal", "Booking") or not tm:
            continue
        mn = e.get("timeMin"); add = e.get("addedTime")
        mm = (f"{mn}+{add}" if add else mn)
        msg = e.get("message") or ""
        if et == "Goal":
            suf = " (PK)" if ("페널티" in msg or "PK" in msg) else (" (자책골)" if "자책" in msg else "")
            goals.append({"p": (e.get("playerName") or "?") + suf, "t": tm, "m": mm})
            rel = e.get("relatedPlayerName")
            if rel and not suf:           # 자책골엔 도움 없음
                assists.append({"p": rel, "t": tm})
        else:  # Booking
            c = "red" if ("퇴장" in msg or "레드" in msg or "두 번째 경고" in msg) else "yellow"
            cards.append({"p": e.get("playerName") or "?", "t": tm, "m": mm, "c": c})
    return goals, assists, cards

def fill_match_events_naver(matches, max_calls=60):
    """완료된 조별 경기의 득점/도움/카드를 네이버 경기 이벤트로 채운다.
       한 번 채운 경기(naverEvents)는 재호출 생략 → 무료/예의 호출 절약."""
    # 완료·득점비어있는 경기만 대상 → 기존 시드(득점 보유)는 그대로 보존
    targets = [m for m in matches if (not m.get("pending")) and (not m.get("naverEvents"))
               and (not m.get("goals")) and m.get("t1") and m.get("t2")]
    if not targets:
        print("  [naver] 경기 이벤트 채울 대상 없음 — 생략")
        return 0
    # 대상 경기 날짜 ±1일(타임존 경계 보정)만 일정 조회
    dates = set()
    for m in targets:
        d = m.get("date")
        if not d:
        ...
        try:
            ...
            dates.add((dt - datetime.timedelta(days=1)).isoformat())
            dates.add((dt + datetime.timedelta(days=1)).isoformat())
        except Exception:
            pass
    idx = naver_schedule_index(dates)
    if not idx:
        print("  [naver] 경기목록을 받지 못함 — 경기별 득점 채우기 생략")
        return 0
    calls = filled = 0
    for m in targets:
        if calls >= max_calls:
            break
        g = idx.get(frozenset([m["t1"], m["t2"]]))
        if not g or not g.get("gameId") or g.get("status") not in NAVER_FINISHED:
            continue
        calls += 1
        res = naver_game_events(g)
        if res is None:
            continue
        goals, assists, cards = res
        m["goals"], m["assists"], m["cards"] = goals, assists, cards
        # 득점을 채웠거나 스코어가 0-0(무득점 확정)일 때만 완료표시 → 이벤트 미반영 경기는 다음 실행에 재시도
        total = (m.get("s1") or 0) + (m.get("s2") or 0)
        if goals or total == 0:
            m["naverEvents"] = True
        filled += 1
    print(f"  [naver] 경기별 득점/도움 {filled}경기 채움 (호출 {calls})")
    return filled
...
def build(existing, raw_matches, source):
    ...
    for r in raw_matches:
        ...
        if p.get("naverEvents"): rec["naverEvents"]=True       # 네이버 경기 이벤트 수집완료 표시 보존(재호출 방지)
        if r["finished"]:
...
def main():
    ...
    # 경기별 득점자·도움·카드 — 1순위: 네이버 경기 이벤트(키 불필요), 폴백: API-Football
    # (라인업 상세와 기존 시드 득점은 그대로 보존, 비어있는 완료 경기만 채움)
    apikey=os.environ.get("APISPORTS_KEY")
    print("[+] 경기별 득점/도움/카드 — 네이버 경기 이벤트 시도…")
    nfilled = fill_match_events_naver(out["matches"])
    if nfilled == 0 and apikey:
        print("[+] 네이버 경기 이벤트 0건 → API-Football 폴백…")
